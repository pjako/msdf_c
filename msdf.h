...
int msdf_genGlyph(msdf_Result* result, stbtt_fontinfo *font, int stbttGlyphIndex, uint32_t borderWidth, float scale, float range, msdf_AllocCtx* alloc) {
    ...
    indices_t *contours = allocCtx.alloc(sizeof(indices_t) * contour_count, allocCtx.ctx);
    int j = 0;
    for (int i = 0; i <= num_verts; i++)
    {
        if (verts[i].type == STBTT_vmove)
    ...
    for (int y = 0; y < h; ++y)
    {
        ...
        for (int x = 0; x < w; ++x)
        {
            ...
            float mr = ((float)msd.r) / range + 0.5f;
            float mg = ((float)msd.g) / range + 0.5f;
            float mb = ((float)msd.b) / range + 0.5f;
            bitmap[index + 0] = mr;
            bitmap[index + 1] = mg;
            bitmap[index + 2] = mb;
